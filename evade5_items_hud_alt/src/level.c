...
// u8 collision_map[MAP_METATILES_W + OFFSCREEN_TILES*2][MAP_METATILES_H + OFFSCREEN_TILES*2] = {0}; // full collision map
...
static u16 LEVEL_mapbuffIDX8(s16 tile_x, s16 tile_y) {
	return (tilemap_buff[tile_y * SCREEN_TILES_W + tile_x]  - map->baseTile) & TILE_INDEX_MASK;
 }

static void LEVEL_set_mapbuffIDX8(s16 tile_x, s16 tile_y, u16 value) {
	tilemap_buff[tile_y * SCREEN_TILES_W + tile_x] = value + map->baseTile;
}

static void LEVEL_print_tilemap_buff() {
...
// void print_level_data2(u16 w, u16 h) {
// 	kprintf("LEVEL DATA2");
// 	// MAP_getTilemapRect(map, 0, 0, w, h, FALSE, tilemap_buff);
// 	// MAP_getMetaTilemapRect(map, 0, 0, SCREEN_METATILES_W, SCREEN_METATILES_H, tilemap_buff);
	
// 	for (u16 x = 0; x < w; ++x) {
// 			for (u16 y = 0; y < h; ++y) {
// 			text_add_int(MAP_getTile(map, x, y) & TILE_INDEX_MASK);
// 			// text_add_int(MAP_getMetaTile(map, x, y) & TILE_INDEX_MASK);
			
// 			// VDP_setTileMapXY(VDP_BG_A, MAP_getTile(map, x, y) + map->baseTile, x, y);
// 		}
// 		text_print_and_clear();
// 	}
// }
...
*/
	// s16 start_x = screen_x/8; // find screen top-left cell in map
	// s16 start_y = screen_y/8;
...
void LEVEL_remove_tileXY(s16 x, s16 y, u8 new_index) {
	...
	// VDP_setTileMapDataRect(?) << replace for another tiles
...
static void LEVEL_scroll_and_update_collision(s16 offset_x, s16 offset_y) {
	// register all existing tiles in current room into BIT MAP
	// >> COLLISION MAP (16x16)
	// << ROOM BIT MAP (16x16)
